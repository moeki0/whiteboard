...
interface GroupProps {
  group: GroupType;
  notes: Note[];
  onSelect: (groupId: string, isMultiSelect?: boolean, isShiftSelect?: boolean) => void;
  isSelected: boolean;
  zoom?: number;
...
function createSmoothPath(points: Array<{ x: number; y: number }>, tension: number = 0.3): string {
  if (points.length < 3) {
    // 点が少ない場合は角丸四角形で繋ぐ
    if (points.length === 2) {
      // 2点の場合は楕円形に近い形状を作る
      ...
      return `M ${centerX - radiusX} ${centerY} 
              Q ${centerX - radiusX} ${centerY - radiusY} ${centerX} ${centerY - radiusY}
              Q ${centerX + radiusX} ${centerY - radiusY} ${centerX + radiusX} ${centerY}
              Q ${centerX + radiusX} ${centerY + radiusY} ${centerX} ${centerY + radiusY}
              Q ${centerX - radiusX} ${centerY + radiusY} ${centerX - radiusX} ${centerY} Z`;
    }
