...
export async function updateBoardSortScore(
  ...
  try {
    ...
    console.log(
      `✅ Updated sortScore for ${boardId}: ${sortScore} (pinned: ${isPinned})`
    );
  } catch (error) {
...
export async function updateAllBoardSortScores(
  ...
  try {
    console.log(
      `🔧 Updating all board sort scores for project ${projectId}...`
    );
    ...
    for (const [boardId, boardData] of Object.entries(boards)) {
      ...
      console.log(
        `📊 ${board.name}: sortScore=${sortScore} (pinned=${isPinned}, updatedAt=${updatedAt})`
      );
    }
    ...
    if (updateCount > 0) {
      ...
      await update(ref(rtdb), updates);
      console.log(
        `✅ Updated ${updateCount} board sort scores in ${(
...
export async function checkSortScoreStatus(projectId: string): Promise<void> {
  try {
    ...
    console.log(
      `📈 Summary: ${withSortScore} with sortScore, ${withoutSortScore} without`
    );

    if (withoutSortScore > 0) {
      console.log(
        "💡 Run updateAllBoardSortScores() to fix missing sortScores"
      );
    }
  } catch (error) {
...
if (import.meta.env.DEV) {
  ...
  console.log(
    "  boardSortScore.update(projectId, boardId, isPinned, updatedAt) - Update single board"
  );
}
