...
export async function getPaginatedBoards(
  ...
  try {
    ...
    try {
      const newStructureResult = await getProjectBoardsList(
        projectId,
        page,
        itemsPerPage
      );
      ...
      const denormalizedBoards: DenormalizedBoard[] =
        newStructureResult.boards.map((item) => ({
          id: item.id,
          name: item.name,
          title: item.title,
      ...
      console.warn(
        "📋 New structure failed, falling back to old structure:",
        error
      );
    }
    ...
    if (denormalizedSnapshot.exists()) {
      ...
      const allBoards = denormalizedSnapshot.val() as Record<
        string,
        DenormalizedBoard
      >;
    ...
    const allBoardEntries = Object.entries(projectBoardsData).map(
    ...
    const validBoards = boardResults.filter(
      (board): board is DenormalizedBoard => board !== null
    );
  ...
  try {
    ...
    if (snapshot.exists()) {
      updates[denormalizedPath] = {
        ...
        ...boardData,
        title:
          boardData.metadata?.title || boardData.name || snapshot.val().title,
        description:
          boardData.metadata?.description ?? snapshot.val().description,
        thumbnailUrl:
          boardData.metadata?.thumbnailUrl ?? snapshot.val().thumbnailUrl,
      };
