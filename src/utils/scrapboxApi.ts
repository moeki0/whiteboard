...
// シンプルなメモリキャッシュ
const scrapboxCache = new Map<string, { data: ScrapboxSearchResult[], timestamp: number }>();
...
export async function searchScrapboxTitles(
  ...
  if (cached && Date.now() - cached.timestamp < CACHE_DURATION) {
    console.log('[Scrapbox API] Returning cached results:', cached.data.length);
    return cached.data;
  }
  ...
  try {
    ...
    console.log('[Scrapbox API] Fetching via Firebase Functions:', url);
    ...
    if (!response.ok) {
      console.warn(`[Scrapbox API] Firebase Functions error: ${response.status} ${response.statusText}`);
      return [];
    }
    
    const data = await response.json();
    console.log('[Scrapbox API] Response data:', data);
    console.log('[Scrapbox API] Response type:', typeof data);
    console.log('[Scrapbox API] Is array:', Array.isArray(data));
    if (Array.isArray(data) && data.length > 0) {
      console.log('[Scrapbox API] First item:', data[0]);
      console.log('[Scrapbox API] First item type:', typeof data[0]);
    }
    ...
    if (Array.isArray(data)) {
      const results = data.map(item => {
        ...
        if (typeof item === 'string') {
          return {
            title: item,
            url: `https://scrapbox.io/${projectName}/${encodeURIComponent(item)}`
          };
        }
        // itemがオブジェクトの場合はtitleプロパティを使用
          ...
          return {
            ...
            url: `https://scrapbox.io/${projectName}/${encodeURIComponent(item.title || '')}`
          };
        }
        // その他の場合は空文字列
        else {
