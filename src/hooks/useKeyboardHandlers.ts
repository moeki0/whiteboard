...
export function useKeyboardHandlers({
  ...
  const handleKeyHintModeProcessing = useCallback((e: KeyboardEvent) => {
    ...
    const matchingEntry = Array.from(noteHintKeys.entries())
      .find(([_, sequence]) => sequence === currentSequence);
    ...
    const hasPartialMatch = Array.from(noteHintKeys.values())
      .some(sequence => sequence.startsWith(currentSequence));
    ...
    return true;
  }, [isKeyHintMode, pressedKeyHistory, setPressedKeyHistory, 
      noteHintKeys, setIsKeyHintMode, setNoteToFocus, setSelectedNoteIds]);
  ...
  const handleEscapeKey = useCallback((e: KeyboardEvent) => {
    if (e.key === "Escape") {
      ...
      if (isKeyHintMode) {
        setIsKeyHintMode(false);
        setPressedKeyHistory([]);
      } else if (selectedNoteIds.size > 0 || selectedItemIds.size > 0 || selectedGroupIds.size > 0) {
        setSelectedNoteIds(new Set());
      }
    }
  }, [isKeyHintMode, setIsKeyHintMode, setPressedKeyHistory, 
      selectedNoteIds, selectedItemIds, selectedGroupIds, setSelectedNoteIds]);
  ...
  const handleShiftKeys = useCallback((e: KeyboardEvent) => {
    if (!e.shiftKey) return false;
    ...
    switch (e.code) {
      ...
      case "KeyS":
        ...
        if (!isKeyHintMode) {
          ...
          const visibleNoteIds = notes.map(note => note.id);
          console.log('Generating hint keys for notes:', visibleNoteIds.length);
          generateHintKeys(visibleNoteIds);
    ...
    return false;
  }, [selectedNoteIds, onCreateGroup, setIsKeyHintMode, notes, generateHintKeys, 
      onAddArrow, onAddNote, setNoteToFocus]);
